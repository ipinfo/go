...
type Core struct {
	...
	CountryFlag CountryFlag  `json:"country_flag,omitempty" csv:"country_flag"`
...
func (v *Core) setCountryName() {
	if v.Country != "" {
		...
		v.CountryFlag.Emoji = GetEmojiFlag(v.Country)
		v.CountryFlag.Unicode = GetEmojiUnicode(v.Country)
	}
